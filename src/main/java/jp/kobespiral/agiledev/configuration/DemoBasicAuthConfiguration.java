package jp.kobespiral.agiledev.configuration;
...
/**
 * WebSecurity などの設定をするためのクラス。 WebSecurityConfigurer インターフェースの実装クラスであり、 抽象クラスである
 * WebSecurityConfigurerAdapter を継承している。
 */
@Configuration
...
public class DemoBasicAuthConfiguration extends WebSecurityConfigurerAdapter {
  ...
  /**
   * WebSecurity の設定をする。 特定のリクエストを無視する設定など、主に全体的なセキュリティ設定を行う。 Spring Boot
   * 起動時に呼び出される。
   *
  ...
  /**
   * HttpSecurity の設定をする。 主に特定の HTTP リクエストに対するセキュリティ設定を実施する。 通常は super で
   * WebSecurityConfigurerAdapter#configure をコールしてはいけない。 デフォルトでは
   * http.authorizeRequests().anyRequest().authenticated().and().formLogin().and().httpBasic();
   * で構成されている。 Spring Boot 起動時に呼び出される。
   *
  ...
  /**
   * AuthenticationManagerBuilder の設定をする。 認証の実装である AuthenticationProvider を指定する。
